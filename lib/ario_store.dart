...
import 'package:ario_store/src/ario_store.dart';
import 'package:ario_store/src/arweave/arweave_service.dart';
import 'package:ario_store/src/entities/collection.dart';
import 'package:ario_store/src/entities/entity.dart';
import 'package:ario_store/src/entities/entity_snapshot.dart';
import 'package:arweave/arweave.dart';
import 'package:graphql/client.dart';

final arioStorage = ArioStore(
  ArweaveService(
      GraphQLClient(
          link: HttpLink('https://arweave.net/graphql'), cache: GraphQLCache()),
      Arweave(gatewayUrl: Uri.parse('https://arweave.net'))),
);
void main() async {
  // getRootFolder();
  ...
  for (var drive in drives) {
    ...
    for (var child in collection.children) {
      if (child is FolderSnapshot) {
        print('┣ ${child.name}');
        final folderChildren = await arioStorage.getCollectionFromFolder(
...
Future<Entity> addNode(EntitySnapshot entity) async {
  if (entity is FolderSnapshot) {
    final children = await arioStorage.getCollectionFromFolder(entity);
